...
<xsl:transform version="1.0"
  ...
  <xsl:template match="m:mets">
	...
	<xsl:value-of select="m:mdWrap/
			      m:xmlData/
			      md:mods/
			      md:titleInfo/@xml:lang"/>
      </xsl:variable>
      <xsl:apply-templates select="m:mdWrap/
				   m:xmlData/
				   md:mods">
  ...
  <xsl:template match="md:mods">
    ...
    <xsl:apply-templates select="md:note[not(@type='pdf')]"/>
    ...
    <xsl:value-of select="IndexLoader:add_field('record_lang',
						...
						'tokenized')"/>
    <xsl:choose>
      <xsl:when test="md:note[@type='pdf']">
